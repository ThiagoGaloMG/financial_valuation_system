...
from database_manager import SupabaseDB # NOVO: Importa o gerenciador do Supabase
...
class IbovespaAnalysisSystem:
    ...
    def __init__(self):
        ...
        self.db = SupabaseDB() # NOVO: Instância do gerenciador do Supabase
    ...
    def _get_companies_data(self, tickers: Optional[List[str]] = None) -> Dict[str, CompanyFinancialData]:
        ...
        Prioriza dados do DB se forem recentes, caso contrário, coleta via yfinance.
        """
        ...
        for ticker in tickers_to_process:
            ...
            # Define um limite de frescor dos dados (ex: 7 dias)
            freshness_threshold = datetime.now() - timedelta(days=7)

            if latest_metrics and datetime.fromisoformat(latest_metrics['metrics']['raw_data']['timestamp_collected']) > freshness_threshold:
                logger.info(f"Usando dados recentes do DB para {ticker}.")
                # Converte de volta para CompanyFinancialData para consistência com o resto do sistema
                # Isso é uma re-hidratação simplificada, pode precisar de mais campos
                companies_data[ticker] = CompanyFinancialData(
                    ...
                    cd_cvm=latest_metrics['metrics']['raw_data'].get('cd_cvm'), # Adicionar se existir
                    sector=latest_metrics['metrics']['raw_data'].get('sector') # Adicionar se existir
                )
            else:
                # Se não houver dados recentes, coleta via yfinance
                logger.info(f"Coletando dados via yfinance para {ticker} (não encontrado ou desatualizado no DB).")
                ...
                if data:
                    ...
                    logger.warning(f"Não foi possível coletar dados para {ticker} via yfinance.")
    ...
    def run_complete_analysis(self, num_companies: Optional[int] = None) -> Dict:
        ...
        if num_companies is None: # Só tenta carregar do DB para análise COMPLETA
            ...
            if latest_report_from_db:
                ...
                if report_date > report_freshness_threshold:
                    ...
                    # Recalcular oportunidades e portfólio, pois eles não são persistidos com a mesma granularidade
                    # e podem ter lógicas dinâmicas
                    companies_data_for_advanced = {}
                    # Re-hidrata os CompanyFinancialData para advanced_ranking
        ...
        # NOVO: Salvar métricas individuais no DB
        for ticker_key, company_data_obj in companies_data.items():
            ...
            # Inclui os raw_data do CompanyFinancialData
        ...
        # NOVO: Salvar o relatório completo no DB
        self.db.save_analysis_report({
            ...
            'execution_time_seconds': (datetime.now() - self.monitor.timers.get("analise_completa_ibovespa", datetime.now())).total_seconds(), # Tempo decorrido
    ...
    def get_company_analysis(self, ticker: str) -> Dict:
        ...
        Tenta buscar as últimas métricas do DB primeiro, se não encontrar, coleta via yfinance.
        """
        self.monitor.start_timer(f"analise_empresa_{ticker}")
        
        # Tenta buscar do DB primeiro
        latest_metrics_from_db = self.db.get_company_latest_metrics(ticker)
        if latest_metrics_from_db:
            # Define um limite de frescor dos dados (ex: 7 dias)
            freshness_threshold = datetime.now() - timedelta(days=7)
            report_date = datetime.fromisoformat(latest_metrics_from_db['metrics']['raw_data'].get('timestamp_collected', datetime.min.isoformat()))
            
            if report_date > freshness_threshold:
                logger.info(f"Usando métricas recentes do DB para {ticker}.")
                return latest_metrics_from_db
            else:
                logger.info(f"Métricas do DB para {ticker} desatualizadas. Coletando via yfinance.")

        # Se não houver dados recentes no DB ou não encontrados, coleta via yfinance
        ...
        try:
            ...
            result = {
                ...
                "metrics": {
                    ...
                    "raw_data": clean_data_for_json({**company_data.__dict__, "timestamp_collected": datetime.now().isoformat()}) # Adiciona timestamp da coleta
                }
            }
            
            # NOVO: Salvar as métricas recém-coletadas e calculadas no DB
    ...
    def get_ibovespa_company_list(self) -> List[Dict]:
        ...
        # Pode ser otimizado para buscar do DB se você tiver uma tabela de tickers persistida
