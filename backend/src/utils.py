...
def format_currency(value: float, currency_symbol: str = "R$") -> str:
    ...
    """
    if abs(value) >= 1e12:
        return f"{currency_symbol} {value/1e12:.2f}T" # Trilhões
    elif abs(value) >= 1e9:
        return f"{currency_symbol} {value/1e9:.2f}B" # Bilhões
    elif abs(value) >= 1e6:
        return f"{currency_symbol} {value/1e6:.2f}M" # Milhões
    elif abs(value) >= 1e3:
        return f"{currency_symbol} {value/1e3:.2f}K" # Milhares
    else:
