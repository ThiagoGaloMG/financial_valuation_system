...
# Também importamos funções de persistência adicionais
try:
...
def run_analysis_endpoint():
    ...
    try:
        ...
        if get_or_create_company and insert_or_update_financial_metrics:
            ...
            try:
                # Se timestamp tiver 'Z' no final, ajustar
                if timestamp_str:
                    # Exemplo: '2025-06-17T16:32:43.379Z' -> Python ISO
                    # Substitui Z por +00:00 para compatibilidade
                    iso = timestamp_str
                    if iso.endswith("Z"):
            ...
            for item in full_data:
                try:
                    ticker = item.get("ticker") or item.get("symbol")  # ajuste se chave for diferente
                    if not ticker:
...
def get_company_analysis(ticker):
    ...
    try:
        ...
        try:
            ...
            # Aqui você pode replicar lógica de persistência por empresa, se desejado
...
# Você pode adicionar outros endpoints de relatórios históricos se desejar,
# por exemplo, listar analysis_reports ou métricas por data.
